...
program packing
  ...
  real(kind=dp) :: x, y, D
  logical :: pack
  integer :: i, j, k, N_circ, N_prev, left, right, above, below, ios
  ...
  !$OMP parallel default(none) shared(box,occupied,N_circ,num_threads,N_prev) &
  !$OMP private(thread_id,i,j,k,x,y,pack,left,right,above,below,rng,D)
  num_threads = OMP_GET_NUM_THREADS()
  thread_id = OMP_GET_THREAD_NUM()
  ! seed the random number generator
  call rng_seed(rng, 362436069+thread_id)
  N_circ = 0
  !$OMP do reduction(+:N_circ)
  do i = 1, 200000000
  ...
  ! print*, "P =", pi*N_circ*r**2/L**2
  print*, omp_get_wtime() - start_time!, "seconds"
